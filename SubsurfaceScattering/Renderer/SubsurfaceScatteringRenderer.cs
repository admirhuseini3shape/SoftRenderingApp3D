...
namespace SubsurfaceScattering.Renderer {
    ...
    public class SubsurfaceScatteringRenderer : ISubsurfaceScatteringRenderer {
        private SubsurfaceScatteringRenderContext _subsurfaceScatteringRenderContext;

        public SubsurfaceScatteringRenderContext SubsurfaceScatteringRenderContext {
            get => _subsurfaceScatteringRenderContext;
            set {
                _subsurfaceScatteringRenderContext = value;
            }
        }
        ...
        public int[] Render() {
            ...
            _subsurfaceScatteringRenderContext.WorldBuffer = worldBuffer;
            ...
            for(var idxVolume = 0; idxVolume < volumeCount; idxVolume++) {
                ...
                for(var idxTriangle = 0; idxTriangle < offsetTriangleCount; idxTriangle++) {
                    ...
                    subSurfacePainter.RendererContext = _subsurfaceScatteringRenderContext;
                ...
                if(!SubsurfaceScatteringRenderUtils.Caries) {
                    if(SubsurfaceScatteringRenderUtils.GaussianBlur)
                        _subsurfaceScatteringRenderContext.Surface.ApplyGaussianBlurToSubsurface();
                    else
                        _subsurfaceScatteringRenderContext.Surface.ApplyClearSubsurface();
                    break;
                }
                ...
                for(var idxTriangle = 0; idxTriangle < cariesTriangleCount; idxTriangle++) {
                    ...
                    cariesPainter.RendererContext = _subsurfaceScatteringRenderContext;
        ...
        public DMesh3 GetDMesh3FromVolume(SubsurfaceScatteringVolume subsurfaceScatteringVolume) {
            return DMesh3Builder.Build(
                subsurfaceScatteringVolume.Vertices, 
                subsurfaceScatteringVolume.Triangles, 
                subsurfaceScatteringVolume.NormVertices.ToFloatArray());
        }

        public void calculateSubsurfaceScattering(VertexBuffer vbx) {
            DMesh3 originalG3 = GetDMesh3FromVolume(vbx.Volume as SubsurfaceScatteringVolume);

            var verticesCount = vbx.Volume.Vertices.Length;

            var lightPos = new Vector3(0, 10, 50);

            (vbx.Volume as SubsurfaceScatteringVolume).InitializeTrianglesColor(ColorRGB.Black);

            var spatial = new DMeshAABBTree3(originalG3);
            spatial.Build();
            for(int i = 0; i < verticesCount; i++) {
                calculateVertexSubsurfaceScattering(vbx.Volume.Vertices[i], lightPos, spatial, originalG3, (vbx.Volume as SubsurfaceScatteringVolume), i);
            }
        }

        public void calculateVertexSubsurfaceScattering(ColoredVertex vertex, Vector3 lightPos, DMeshAABBTree3 spatial, DMesh3 originalG3, SubsurfaceScatteringVolume subsurfaceScatteringVolume, int index) {
