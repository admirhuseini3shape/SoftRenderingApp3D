...
namespace SoftRenderingApp3D.App.Utils {
    public static class DisplayModelHelpers {
        ...
        private static readonly Dictionary<string, FileReader> _readers = new Dictionary<string, FileReader>
        {
            { Constants.Readers.ColladaReader, new ColladaReader() },
            { Constants.Readers.StlReader, new STLReader() }
        };

        private static readonly Dictionary<string, Action<World>> _generatingMethods = new Dictionary<string, Action<World>>
        {
