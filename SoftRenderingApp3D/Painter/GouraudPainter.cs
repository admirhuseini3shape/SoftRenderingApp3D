    ...
    public class GouraudPainter : IPainter {
        ...
        void paintScanline(float y, float sx, float ex, float sz, float ez, float sl, float el, Vector3 zWorld) {
            ...
            for(var x = minX; x < maxX; x++) {
                ...
                var subsurfaceColor = RenderUtils.subsurfaceColor;

                var finalColor = RenderUtils.surfaceOpacity * (RenderUtils.lightWeight * c * surfaceColor + RenderUtils.subsurfaceScatteringWeight * subsurfaceColor);

                surface.PutPixel((int)x, (int)y, (int)z, finalColor, zWorld);
            }
        }

        ColorRGB InterpolateTriangleVertexColors(Triangle triangle, int y,)
    }
}
